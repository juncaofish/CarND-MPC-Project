...
class FG_eval {
 public:
  ...
  void operator()(ADvector &fg, const ADvector &vars) {
    ...
    // Objective term 3:  actuators smoothness in change
    for (size_t t = 0; t < N - 2; ++t) {
      fg[0] += steer_change_cost_weight * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += acc_change_cost_weight * CppAD::pow(vars[a_start     + t + 1] - vars[a_start     + t], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  double max_radians  = deg2rad(max_degree);// * M_PI / 180;
  ...
  CppAD::ipopt::solve<Dvector, FG_eval>(
    options, vars, vars_lower_bounds, vars_upper_bounds,
    constraints_lower_bounds, constraints_upper_bounds, fg_eval, solution);
